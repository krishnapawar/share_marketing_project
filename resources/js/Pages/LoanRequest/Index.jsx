...
const Index = ({ auth }) => {
    ...
    const { delete: destroy, get, post,put, reset, errors,data, setData, processing,recentlySuccessful } = useForm({
        status: '',
        remark: ''
    });
    ...
    const approvedLoanStatusSubmit = async(loanRequest) =>{
        const isConfirmed = await confirm(
            `Are you sure you want to approved the Loan Request #${loanRequest.id}? This action
            is irreversible.`);
            if (isConfirmed) {
    ...
    const columns =[
        { label: 'ID', key: 'id' },
        { label: 'CreatedAt', key: 'created_at', render:(loanRequest)=>{
            return new Date(loanRequest.created_at).toLocaleString()
        } },
        { label: 'Customer Name', render:(data)=>{
            return data.user.name ?? '-';
        }},
        { label: 'Remark', render:(data)=>{
            return data.remark ?? '-';
        }},
        { label: 'Status', key: 'status', render:(loanRequest)=>{
            return (
                <Link onClick={(e)=> {e.preventDefault(); loanRequest.status!='approved' && approvedLoanStatusModal(loanRequest)}} className={`${loanRequest.status}  mt-1 block w-full `}>{loanRequest.status}</Link>
            );
        } },
  ...
  return (
    <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Loan Request</h2>}
    >
        <div className="py-12">
            <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div className="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    ...
                    <div className="p-6 text-gray-900 dark:text-gray-100">
                        <Table columns={columns} data={loanRequests.data} paginate={loanRequests} />
                    </div>
                </div>
            </div>
        </div>
        ...
        <Modal
            ...
            >
                ...
                >
                    ...
                    <div className="p-6 text-gray-900 dark:text-gray-100">
                        <form onSubmit={(e) => { e.preventDefault(); approvedLoanStatusSubmit(approvedLoanModelData); }}>
                            ...
                            <ModalFooter>
                                <div className="flex items-center gap-4 mt-4">
                                    <SecondaryButton type="button" disabled={processing} onClick={approvedLoanStatusModalClose}>Close</SecondaryButton>
                                </div>
                                <div className="flex items-center gap-4 mt-4">
